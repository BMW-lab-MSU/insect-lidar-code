...
load('D:\Data-Test\2016-06-08\processed_data\events\manual.mat');

if exist('D:\Data-Test\2016-06-08\processed_data\events\manual_insects.mat','file')~=0
    load('D:\Data-Test\2016-06-08\processed_data\events\manual_insects.mat');
...
for n=1:size(manual.insects,2)
    ...
    while strcmp(more,'y')==1
        clear data fn
        % load in the run
        data=load(fullfile(pwd,fols(manual.insects(n).name));
        fn=fieldnames(data);
        figure(1)
        % display the run
