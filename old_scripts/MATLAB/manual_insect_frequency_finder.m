function []=manual_insect_frequency_finder()
set_plot_defaults
...
try
    load('C:\Users\user\Documents\Research\Insect Lidar\Field Tests\Stored Data\2016-06-08\processed_data\events\manual_final_insect.mat')
catch
    disp('no mfi yet');
end
try
...
for n=n_start:size(manual_insects,2)
    ...
    if lower_bound==0
        ...
        %         [gminv,gmini]=findpeaks(1.01*max(abs(gfreq_data(513:end)).^2)-abs(gfreq_data(513:end)).^2);
        gmovmx=movmax((abs(CB.gfreq_data(513:end))).^2,round(sl_coef*mean(diff(gmaxi(2:end)))));
        %         [imxtf,imx]=ismember(gmaxv,gmovmx);
        ...
        tcb.tax=plot(data.(fn{:})(manual_insects(n).y).time,np,data.(fn{:})(manual_insects(n).y).time,np,'b',...
            ...
            mxtime,mxval,'ko'); %,data.(fn{:})(manual_insects(n).y).time(loc),np(loc),'m*'
        title(sprintf('Windowed - AMK Ranch on %s\n at a distance of %2.1f m',...
            exact_time, data.(fn{:})(manual_insects(n).y).range(manual_insects(n).z)))
        xlim([data.(fn{:})(manual_insects(n).y).time(lower_bound)...
            data.(fn{:})(manual_insects(n).y).time(upper_bound)])
%         text(data.(fn{:})(manual_insects(n).y).time(lower_bound),...
%             max(tcb.positive_data),num2str(size(mxval,1)))
%         
        xlabel('time (s)')
        ylabel('normalized detected signal (arb)')
...
CB.gmaxi=gmaxi;
CB.igmx=igmx;
% set(CB.gfax(3),'ydata',gmovmx)
% disp(rndval)

function []=SliderTCB(source,event)
global tcb
wnd_ts_th=source.Value;
%         cur_loc=find(tcb.positive_data(1:tcb.upper_bound)>=wnd_ts_th*median(tcb.positive_data),1,'first');
...
%     text(tcb.data.(tcb.fn{:})(tcb.manual_insects(tcb.n).y).time(lower_bound),max(tcb.positive_data),num2str(size(mxval,1)))
...
% temp=get(slider, 'Value');
% get(gfax(2),'Ydata')
% set(gfax(2),'Ydata',umx(indx_igmx).*temp)
