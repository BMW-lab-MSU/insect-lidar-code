function full_stats=Insect_Lidar_Statistics_Finder_TH2(date,sf)
    ...
    if ~isempty(pd_loc)
        for n=pd_loc
            ...
            for m=temp_location_var'
                ...
                ccc=find(m==[full_stats.(statfield{sf})(full_stats_curlength:full_stats_curlength+length(fn_abs)-1).location]);
%                 if events.temp.loc(m)==1;
                    full_stats.(statfield{sf})(full_stats_curlength-1+ccc).ratio=events.temp.(field_name{:})(cc).ratio(m,:);
                    full_stats.(statfield{sf})(full_stats_curlength-1+ccc).max_ratio=min(events.temp.(field_name{:})(cc).ratio(m,:));
                    full_stats.(statfield{sf})(full_stats_curlength-1+ccc).time=processed.(field_name{:})(n).time;
                    full_stats.(statfield{sf})(full_stats_curlength-1+ccc).location_mean=events.temp.mean(m,:);
%                 else
