...
if strcmp('scan',type)
    tl=150;
    tld=tl;
    start_pan=3900; %2000; % 100*degrees
    end_pan=4250; %2000;   % 100*degrees
    pan_s_e=[start_pan end_pan];
    delta_pan=50; % 100*degrees
    delta_tilt=50; % 100*degrees
    tilt_final=1500; % 100*degrees
    ...
    while tld<tilt_final;
        ...
        while (-1)^(n+1)*pld<(-1)^(n+1)*pan_s_e(nn)
            ...
            %             clear sep_save
            %             sep_data=insect_lidar(q).data;
%             clear start_address
%             start_address=actual.ActualStart;
            save(fullfile(time_dir,[sprintf('%05.0f',q),'_','P',sprintf('%04.0f',pli),'T',sprintf('%04.0f',tli)]),'full_data','start_address','tsdata','panloc','tiltloc');
...
% save(fullfile(time_dir,filename),'insect_lidar')
