...
for pn=1:size(tilt_vec,2)
    ...
    name_file{pn}=nf;
    for rn = rn_vec
        clear processed_data pm
        load(fullfile(date_dir,runs(rn).name,'processed_data.mat'))
        pm=find(abs([processed_data.tilt]-tilt_vec(pn))<0.24 & abs([processed_data.pan]-pan_vec(pn))<0.24);
        if exist('pm','var')
        for spm=1:size(pm,2)
            vector_data.(name_file{pn})(int)=processed_data(pm(spm));
            int=int+1;
        end
        end
