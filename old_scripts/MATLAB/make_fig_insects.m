                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                function analysis_data=make_fig_insects(vli_start)
close all
set_plot_defaults
...
try
load(fullfile(start_file,'Figures','Insect_Figures','Profiles','repeat_check.mat'));
catch
...
% vli=1;
try
    load(fullfile(start_file,'Figures','Insect_Figures','Profiles','analysis_data.mat'))
...
for vli=vli_start:size(manual_final_insect.very_likely,2)
    ...
    data=load(fullfile(filename,fols(manual_final_insect.very_likely(vli).x).name));
    fn=fieldnames(data);
    tfg=dir(fullfile(start_file,'Figures','Insect_Figures','Profiles'));
    ...
    positive_data=abs(data.(fn{:})(manual_final_insect.very_likely(vli).y).normalized_data(manual_final_insect.very_likely(vli).z,:)-...
        max(data.(fn{:})(manual_final_insect.very_likely(vli).y).normalized_data(manual_final_insect.very_likely(vli).z,:)));
    ...
    fig_data.time_data=data.(fn{:})(manual_final_insect.very_likely(vli).y).time;
    fig_data.range_data=data.(fn{:})(manual_final_insect.very_likely(vli).y).range;
    fig_data.x=manual_final_insect.very_likely(vli).x;
    fig_data.y=manual_final_insect.very_likely(vli).y;
    fig_data.z=manual_final_insect.very_likely(vli).z;
    ...
    if strcmp(skypyn,'y')
        ...
        rmdir(fullfile(start_file,'Figures','Insect_Figures','Profiles',fig_value),'s')
    end
    if ~strcmp(skypyn,'n')
        ...
        int=int+1;
        analysis_data(int).data=data.(fn{:})(manual_final_insect.very_likely(vli).y).normalized_data(manual_final_insect.very_likely(vli).z,:);
        analysis_data(int).tilt=data.(fn{:})(manual_final_insect.very_likely(vli).y).tilt;
        analysis_data(int).pan=data.(fn{:})(manual_final_insect.very_likely(vli).y).pan;
        analysis_data(int).time=data.(fn{:})(manual_final_insect.very_likely(vli).y).time;
        analysis_data(int).range=data.(fn{:})(manual_final_insect.very_likely(vli).y).range(manual_final_insect.very_likely(vli).z);
        analysis_data(int).filename=data.(fn{:})(manual_final_insect.very_likely(vli).y).filename;
    ...
    close all
    save(fullfile(start_file,'Figures','Insect_Figures','Profiles','repeat_check.mat'),'repeat_check','-v7.3')
    save(fullfile(start_file,'Figures','Insect_Figures','Profiles','analysis_data.mat'),'analysis_data','-v7.3')
end
