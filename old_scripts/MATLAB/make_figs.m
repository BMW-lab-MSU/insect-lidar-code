...
date='2016-06-08';
time='AMK_Ranch-210853';
filename=fullfile(start_file,'Stored Data',date,time);
load(fullfile(filename,'adjusted_data.mat'))
tfg=dir(fullfile(start_file,'Figures','Thesis Figures'));
tfg=tfg(~ismember({tfg.name},{'.','..'}));
tfg=tfg([tfg.isdir]);
for n=1:length(tfg);tfgname(n)=str2num(tfg(n).name(5:length(tfg(n).name)));end
nfv=max(tfgname);

fig_value=sprintf('fig_%2i',nfv+1);
start_file='C:\Users\user\Documents\Research\Insect Lidar\Field Tests\';
date='2016-06-08';
time='AMK_Ranch-195944';
filename=fullfile(start_file,'Stored Data',date,time);
load(fullfile(filename,'adjusted_data.mat'))
...
positive_data=abs(adjusted_data(n).normalized_data(m,:)-max(adjusted_data(n).normalized_data(m,:)));
% positive_data=(adjusted_data(m,:)-min(adjusted_data(m,:)));
fig_data.positive_data=positive_data;
fig_data.time_data=adjusted_data(n).time;
fig_data.range_data=adjusted_data(n).range;
...
if exist(fullfile(start_file,'Figures\Thesis Figures\',fig_value),'dir')==7
...
if dofreq==0
    if dogauss==1
        ...
        xlabel('time (s)')
        ylabel('normalized detected signal (arb)')
        title(sprintf('Insect at AMK Ranch on %s\nat %s at a distance of %2.1f m', fig_data.date,fig_data.time, fig_data.range_data(fig_data.location_data)))
        savefig(fullfile(start_file,'Figures','Thesis Figures',fig_value,[fig_value, '.fig']));
        saveas(gcf,fullfile(start_file,'Figures','Thesis Figures',fig_value,[name,'.png']));
        figure(2)
    ...
    savefig(fullfile(start_file,'Figures','Thesis Figures',fig_value,[fig_value, '.fig']));
    saveas(gcf,fullfile(start_file,'Figures','Thesis Figures',fig_value,[name,'f.png']));
end
