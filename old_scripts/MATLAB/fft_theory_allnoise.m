...
indx_spc=time(20+1);
indx_mul=[];
noise=rand(1,1024)*.50*time(20+1);

zgaus=gaussmf(time,[time(sv+1),time(100+1)]);
for n=1:user_def
    indx_mul=[indx_mul n*indx_spc+time(randi([0 4],1)+1)*sign(rand()-1/2)];
    sp=gaussmf(time,[time(sv+1+randi([-1 4],1)),indx_mul(n)+time(200+1)])*time(20+1);
    peaks=peaks+sp;
end
for n=1:size(time,2)
    if peaks(n)<0.004
