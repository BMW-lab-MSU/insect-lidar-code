...
if exist('/Volumes/Insect Lidar/Data_2020/2020-09-20/events/sigcheck_maybe.mat','file')~=0
    load('/Volumes/Insect Lidar/Data_2020/2020-09-20/events/sigcheck_maybe.mat');
    goodinput=0;
    int=0;%int=length(sigcheck_maybe.insects); 
    nnt=length(sigcheck_maybe.noninsects); %nnt=0;
    mnt=length(sigcheck_maybe.maybe_insects);
    start=sigcheck_maybe.lastcompleted+1;
else
    int=0;
    nnt=0;
    mnt=0;
...
if exist('/Volumes/Insect Lidar/Data_2020/2020-09-20/events/fftcheck_maybe.mat','file')~=0
    load('/Volumes/Insect Lidar/Data_2020/2020-09-20/events/fftcheck_maybe.mat');
    checkinput=0;
    hnt=0;%hnt=length(fftcheck_maybe.insects);
    jnt=0;%jnt=length(fftcheck_maybe.noninsects); %jnt=0;
    knt=length(fftcheck_maybe.maybe_insects);
    startf=fftcheck_maybe.lastcompleted+1;
else
    hnt=0;
    jnt=0;
    knt=0;
...
for n = start:size(manual.maybe_insect,2) %CHANGE THIS PRIOR TO RUNNING
    close all
    whichfol=manual.maybe_insect(n).foldernum;
    whichfile=manual.maybe_insect(n).filenum;

    % first identify range bin of interest
    % do imagesc then ask for input on range bin
    
    % Load relevant data 
    load(fullfile(filename,manual.maybe_insect(n).name,'adjusted_data_decembercal'));
    ...
    f1 = figure(1);
    movegui('north');
    title(sprintf('Folder: %s File: %i',manual.maybe_insect(n).name,whichfile),'interpreter','none');
    xlabel('Pulse Number');
    ...
    while strcmp(more,'y')==1
        ...
        movegui('northeast');
        xlim([0 1024])
        title(sprintf('Folder:  %s File: %i  Range Bin: %i',manual.maybe_insect(n).name,whichfile,loc2),'interpreter','none');
        xlabel('Pulse Number');
        ...
        if strcmp(yn,'q') %yes insect present
            int=int+1;
            goodinput=1;
            sigcheck_maybe.insects(int).name=fols(whichfol).name;
            sigcheck_maybe.insects(int).foldernum=whichfol;
            sigcheck_maybe.insects(int).filenum=whichfile;
            sigcheck_maybe.insects(int).range=loc2;
            sigcheck_maybe.insects(int).lb=lower;
            sigcheck_maybe.insects(int).ub=upper;
        elseif strcmp(yn,'w') % no insect present
            goodinput=1;
            nnt=nnt+1;
            sigcheck_maybe.noninsects(nnt).name=fols(whichfol).name;
            sigcheck_maybe.noninsects(nnt).foldernum=whichfol;
            sigcheck_maybe.noninsects(nnt).filenum=whichfile;
            sigcheck_maybe.noninsects(nnt).range=loc2;
        elseif strcmp(yn,'e') %maybe insect present
            goodinput=1;
            mnt=mnt+1;
            sigcheck_maybe.maybe_insects(mnt).name=fols(whichfol).name;
            sigcheck_maybe.maybe_insects(mnt).foldernum=whichfol;
            sigcheck_maybe.maybe_insects(mnt).filenum=whichfile;
            sigcheck_maybe.maybe_insects(mnt).range=loc2;
            sigcheck_maybe.maybe_insects(mnt).lb=lower;
            sigcheck_maybe.maybe_insects(mnt).ub=upper;
        else
        ...
        movegui('southwest');
        % Load relevant data 
        %load(fullfile(filename,sigcheck_maybe.insects(m).name,'adjusted_data_decembercal'));
        ...
        if strcmp(isinsect,'q') %yes insect present
            hnt=hnt+1;
            checkinput=1;
            fftcheck_maybe.insects(hnt).name=fols(whichfol).name;
            fftcheck_maybe.insects(hnt).foldernum=whichfol;
            fftcheck_maybe.insects(hnt).filenum=whichfile;
            fftcheck_maybe.insects(hnt).range=loc2;
            fftcheck_maybe.insects(hnt).lb=lower;
            fftcheck_maybe.insects(hnt).ub=upper;
            maybefreq=str2double(input('Best guess frequency? ','s'));
            fftcheck_maybe.insects(hnt).freq=maybefreq;
        elseif strcmp(isinsect,'w') % no insect present
            checkinput=1;
            jnt=jnt+1;
            fftcheck_maybe.noninsects(jnt).name=fols(whichfol).name;
            fftcheck_maybe.noninsects(jnt).foldernum=whichfol;
            fftcheck_maybe.noninsects(jnt).filenum=whichfile;
            fftcheck_maybe.noninsects(jnt).range=loc2;
        elseif strcmp(isinsect,'e') %maybe insect present
            checkinput=1;
            knt=knt+1;
            fftcheck_maybe.maybe_insects(knt).name=fols(whichfol).name;
            fftcheck_maybe.maybe_insects(knt).foldernum=whichfol;
            fftcheck_maybe.maybe_insects(knt).filenum=whichfile;
            fftcheck_maybe.maybe_insects(knt).range=loc2;
            fftcheck_maybe.maybe_insects(knt).lb=lower;
            fftcheck_maybe.maybe_insects(knt).ub=upper;
%             maybefreq=str2double(input('Best guess frequency? ','s'));
%             fftcheck_maybe.maybe_insects(knt).freq=maybefreq;
        else
        ...
        end % end of checking if sigcheck_maybe has insect or not in order to run fft
...
sigcheck_maybe.lastfilename=fols(whichfol).name;
sigcheck_maybe.lastcompleted=n;
save(fullfile(pwd,'events','sigcheck_maybe.mat'),'sigcheck_maybe','-v7.3')

fftcheck_maybe.lastfilename=fols(whichfol).name;
fftcheck_maybe.lastcompleted=n;
save(fullfile(pwd,'events','fftcheck_maybe.mat'),'fftcheck_maybe','-v7.3')
