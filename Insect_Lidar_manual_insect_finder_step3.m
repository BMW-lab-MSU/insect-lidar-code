...
for n=n_start:size(manual_insects,2)
    ...
    %positive_data=abs(data.(fn{:})(manual_insects(n).y).normalized_data(manual_insects(n).z,:)...
     ...
     positive_data = data.(fn{:})(manual_insects(n).y).normalized_data(manual_insects(n).z,:);
    ...
    %set(figone,'outerposition',[633 213 656 688])
    ...
    if lower_bound==0
        ...
        delta_t=mean(diff(data.(fn{:})(manual_insects(n).y).time)); %pulse time difference
