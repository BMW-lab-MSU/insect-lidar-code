...
prc_dir = 'G:\Data-Test\2016-06-08\processed_data';
prc_file_list = dir(fullfile(prc_dir,'*T*.mat'));
%prc_file_list = dir(fullfile(prc_dir,'*T1_50P39_50.mat'));

% Load SVM
file_dir = 'G:\scripts\insect-lidar\Support vector machine';
svm_file = dir(fullfile(file_dir,'*SVM_Model_insect_new2*.mat'));
svm = load(fullfile(file_dir,svm_file.name));

image_dir = 'G:\scripts\insect-lidar\Support vector machine\Images_SVMProcessor10'; %Change destination for each run or data will be overridden******



%% Variables and filters
pp = 0; %Table increment
pn = 0; %Structure increment 
mn = 0;
sn = 0; 
...
%         positive_hard_target = positive_hard_target+prc_file.(prc_name{1})(k).normalized_data;
% 
%     end
...
%         min_pos = min(min(prc_file.(prc_name{1})(b).normalized_data));
        %positive_data = abs(min_pos) + prc_file.(prc_name{1})(b).normalized_data;
         positive_data = prc_file.(prc_name{1})(b).normalized_data;
...
%         positive_data_no_t = positive_data(positive_gauss_norm<0.5);
        ...
        % Store and sum data
        bin_num = 1:1:178; % Sum 2 range bins
...
% figure(1)
%             imagesc(prc_file.T1_50P39_50(b).normalized_data)
%             figure(2)
%             plot(positive_stacked(kk,:))
...
%                 xticks(time_data)
                subplot(4,2,7),imagesc(positive_data_no_t);
                title('Hard Targets');
                xticks(time_data)
...
%                 title('Frequency Spectrum');
    ...
    %             set(f,'Position',[500 500 300 150]);
                ...
                %Structure to save SVM Processor Statistics 
%                 pn = pn+1; 
%                 
%                 processor_stats(pn).Filename = prc_file.(prc_name{1})(b).filename;
%                 processor_stats(pn).PredictValue = label_var; 
%                 processor_stats(pn).MaxPeak = test_class_ex(1);
%                 processor_stats(pn).MaxAmp = test_class_ex(2);
%                 processor_stats(pn).Kurtosis = test_class_ex(3);
%                 processor_stats(pn).PeakDeviation = test_class_ex(4);
%                 processor_stats(pn).PeakRatio = test_class_ex(5);
%                 processor_stats(pn).NumberofPeaks = test_class_ex(6);
%                 processor_stats(pn).HarmonicMean = test_class_ex(7);
%                 
%                 save(fullfile(image_dir,['processor_stats','.mat']),'processor_stats','-v7.3');
%                 
            ...
            %    Create table
                ...
                if label_var > 0.90 && max(non_amp_peaks)>0.5
                    saveas(gcf,fullfile(image_dir,'Very_likely',['predict_value =' strrep(num2str(label_var), '.', '')]), 'png'); 
                   
                    pn = pn+1; 
                
             VeryLikely_stats(pn).Filename = prc_file.(prc_name{1})(b).filename;
              format long;
             VeryLikely_stats(pn).PredictValue = label_var; 
             VeryLikely_stats(pn).MaxPeak = test_class_ex(1);
             VeryLikely_stats(pn).MaxAmp = test_class_ex(2);
             VeryLikely_stats(pn).Kurtosis = test_class_ex(3);
             VeryLikely_stats(pn).PeakDeviation = test_class_ex(4);
             VeryLikely_stats(pn).PeakRatio = test_class_ex(5);
             VeryLikely_stats(pn).NumberofPeaks = test_class_ex(6);
             VeryLikely_stats(pn).HarmonicMean = test_class_ex(7);
                
                save(fullfile(image_dir,['VeryLikely_stats','.mat']),'VeryLikely_stats','-v7.3');
                
                elseif label_var > 0.8 && max(non_amp_peaks)>0.5
                    saveas(gcf,fullfile(image_dir,'Most_likely',['predict_value =' strrep(num2str(label_var), '0.', '')]),'png');
                    
                    mn = mn+1; 
                
              MostLikely_stats(mn).Filename = prc_file.(prc_name{1})(b).filename;
              MostLikely_stats(mn).PredictValue = label_var; 
              MostLikely_stats(mn).MaxPeak = test_class_ex(1);
              MostLikely_stats(mn).MaxAmp = test_class_ex(2);
              MostLikely_stats(mn).Kurtosis = test_class_ex(3);
              MostLikely_stats(mn).PeakDeviation = test_class_ex(4);
              MostLikely_stats(mn).PeakRatio = test_class_ex(5);
              MostLikely_stats(mn).NumberofPeaks = test_class_ex(6);
              MostLikely_stats(mn).HarmonicMean = test_class_ex(7);
                
                save(fullfile(image_dir,['MostLikely_stats','.mat']),'MostLikely_stats','-v7.3');
                
                elseif label_var > 0.7 && max(non_amp_peaks)>0.5
                    saveas(gcf,fullfile(image_dir,'Somewhat_likely',['predict_value =' strrep(num2str(label_var), '0.', '')]),'png');
                     sn = sn+1; 
                
                SomewhatLikely_stats(sn).Filename = prc_file.(prc_name{1})(b).filename;
                SomewhatLikely_stats(sn).PredictValue = label_var; 
                SomewhatLikely_stats(sn).MaxPeak = test_class_ex(1);
                SomewhatLikely_stats(sn).MaxAmp = test_class_ex(2);
                SomewhatLikely_stats(sn).Kurtosis = test_class_ex(3);
                SomewhatLikely_stats(sn).PeakDeviation = test_class_ex(4);
                SomewhatLikely_stats(sn).PeakRatio = test_class_ex(5);
                SomewhatLikely_stats(sn).NumberofPeaks = test_class_ex(6);
                SomewhatLikely_stats(sn).HarmonicMean = test_class_ex(7);
                
                save(fullfile(image_dir,['SomewhatLikely_stats','.mat']),'SomewhatLikely_stats','-v7.3');
                end
