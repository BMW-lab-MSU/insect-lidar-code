...
% [status.setSimpleTrigger] = invoke(triggerGroupObj, 'setSimpleTrigger', 1, 500, 2);
...
TriggerInfo(i)=ps5000aStructs.tPS5000ATriggerInfo.members;
TriggerInfo(i).status=PicoStatus.PICO_DATA_NOT_AVAILABLE;
TriggerInfo(i).segmentIndex=0;
TriggerInfo(i).triggerIndex=0;
TriggerInfo(i).triggerTime=0;
TriggerInfo(i).timeUnits=0;
TriggerInfo(i).reserved0=0;
TriggerInfo(i).timeStampCounter=0;
end

TriggerInfoPtr = libpointer('tPS5000ATriggerInfoPtr',TriggerInfo)
%%
status.getTriggerInfoBulk = calllib('ps5000a', 'ps5000aGetTriggerInfoBulk', ps5000aDeviceObj.DriverData.unitHandle, TriggerInfoPtr, fromSegmentIndex, toSegmentIndex);
%%
TriggerInfoOut(1)=TriggerInfoPtr.Value;
for i = 1:(toSegmentIndex-fromSegmentIndex)
TriggerInfoPtrInc = TriggerInfoPtr+i;
TriggerInfoOut(i+1)=TriggerInfoPtrInc.Value;
end
...
%[timestamp] = invoke(triggerGroupObj,'ps5000aGetTriggerInfoBulk'); 
