...
close all
op=[1673 33 1696 1026];
set_plot_defaults
start_file='D:\Data-Test\';
date='2016-06-08';
filename=fullfile(start_file,date,'processed_data');
fols=dir(filename);
...
for vli=vli_start:size(manual_final_insect.somewhat_likely,2)
    clearvars -except op start_file date filename fols manual_final_insect repeat_check vli int analysis_data
    data=load(fullfile(filename,fols(manual_final_insect.somewhat_likely(vli).x).name));
    fn=fieldnames(data);
    ...
    exact_dnum=datenum([date,'-',num2str(str2num(data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).filename(11:16))+...
        str2num(datestr(mode_run_time*(str2num(data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).filename(18:22))/230),'HHMMSS')))],...
        'yyyy-mm-dd-HHMMSS');
    
    exact_time=datestr(exact_dnum,'yyyy-mm-dd HH:MM:SS');
    %%
    fig_data.positive_data=positive_data;
    fig_data.time_data=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).time;
    fig_data.range_data=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).range;
    fig_data.x=manual_final_insect.somewhat_likely(vli).x;
    fig_data.y=manual_final_insect.somewhat_likely(vli).y;
    fig_data.z=manual_final_insect.somewhat_likely(vli).z;
    ...
    if vli~=1
        if repeat_check(vli).dnum==repeat_check(vli-1).dnum && repeat_check(vli).z==repeat_check(vli-1).z+1
            fig1=figure(1);
%             subplot(2,1,1)
            plot(1:length(fig_data.positive_data),fig_data.positive_data,'b',1:length(fig_data.positive_data),repeat_check(vli-1).data.positive_data,'r')
            xlabel('pulse number')
            ylabel('normalized detected signal (arb)')
            title('data')
            legend('cd','pd','location','best')
...
%             title('past data')
            ...
            set(fig1,'outerposition',op)
            
            skypyn='q';
            while ~strcmp(skypyn,'pd') && ~strcmp(skypyn,'cd') && ~strcmp(skypyn,'x')
                skypyn=input('use cd or pd? (x if not a repeat) ','s');
            end
        else
            fig1=figure(1);
    ...
    if ~strcmp(skypyn,'pd')
        ...
        %         if exist(fullfile(start_file,'Figures\SL_Insect_Figures\Profiles',fig_value),'dir')==7
        %             save(fullfile(start_file,'Figures','SL_Insect_Figures','Profiles',fig_value,'fig_data.mat'),'fig_data','-v7.3')
        %         else
        %             mkdir(fullfile(start_file,'Figures','SL_Insect_Figures','Profiles',fig_value))
        %             save(fullfile(start_file,'Figures','SL_Insect_Figures','Profiles',fig_value,'fig_data.mat'),'fig_data','-v7.3')
        %         end
        med=median(fig_data.positive_data);
        maxpd=max(fig_data.positive_data);
        cutoff=(maxpd*.15)+med;
        hold on
        plot(1:length(fig_data.positive_data),cutoff.*ones(size(fig_data.positive_data)));
        hold off
        lowerbound=find(fig_data.positive_data>=cutoff,1,'first');
        if lowerbound-100>0
            lsec=lowerbound-100;
        else
            lsec=0;
        end
        upperbound=find(fig_data.positive_data>=cutoff,1,'last');
...
%         fig1=figure(1);
%         subplot(2,2,1)
%         plot(fig_data.time_data,fig_data.positive_data,'b',fig_data.time_data,cutoff.*ones(size(fig_data.time_data)),'r')
%         xlabel('time (s)')
%         ylabel('normalized detected signal (arb)')
%         title(sprintf('Insect at AMK Ranch on %s\nat a distance of %2.1f m', fig_data.date_time, fig_data.range_data(fig_data.z)))
%         set(fig1,'outerposition',op)
...
%         pause(1.5)
        ...
        int=int+1;
        analysis_data(int).data=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).normalized_data(manual_final_insect.somewhat_likely(vli).z,:);
        analysis_data(int).tilt=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).tilt;
        analysis_data(int).pan=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).pan;
        analysis_data(int).time=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).time;
        analysis_data(int).range=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).range(manual_final_insect.somewhat_likely(vli).z);
        analysis_data(int).filename=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).filename;
        ...
        analysis_data(int).max_peak=pk_freq;
        analysis_data(int).positive_data=fig_data.positive_data;
        disp('done')
    end
    close all
