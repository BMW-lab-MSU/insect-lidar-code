...
fid = fopen(fullfile(tempgage,filename), 'w');
if -1 == fid
    ret = -1;
    return;
end;
...
if nargin > 2
    ...
    if isfield(headerinfo, 'TimeStamp')
        fprintf(fid, 'Time stamp     = %.2f\r\n', headerinfo.TimeStamp); %microseconds
    end;
    if isfield(headerinfo, 'Pan')
        fprintf(fid, 'Pan location   = %4.2f\r\n', headerinfo.Pan);
    end;
    if isfield(headerinfo, 'Tilt')
        fprintf(fid, 'Tilt location  = %4.2f\r\n', headerinfo.Tilt);
    end;
    fprintf(fid, '------------------\r\n');
end;
