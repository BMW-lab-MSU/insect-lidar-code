...
t = exist(fullfile(filename, 'Training_Data'),'dir');

if t == 0
    
mkdir(filename, 'Training_Data');
addpath(fullfile(filename,'Training_Data'));
...
for vli=vli_start:size(manual_final_insect.somewhat_likely,2)
     ...
     clearvars -except start_file date filename fols events manual_final_insect vli int sl mode_run_time data fn vli_start t
    
    data=load(fullfile(filename,fols(manual_final_insect.somewhat_likely(vli).x).name)); %information from filename selected
...
positive_data = data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).normalized_data...
    (manual_final_insect.somewhat_likely(vli).z,:);
...
%frequency%
 ...
 time_data=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).time;   %collects the time data from each file
...
%max_peak%
 ...
 pk_freq=fqdata(lbfreq+freq_i-1);
      ...
      int=int+1;
        sl.analysis_data(int).data=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).normalized_data(manual_final_insect.somewhat_likely(vli).z,:);
        sl.analysis_data(int).tilt=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).tilt;
        sl.analysis_data(int).pan=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).pan;
        sl.analysis_data(int).time=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).time;
        sl.analysis_data(int).range=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).range(manual_final_insect.somewhat_likely(vli).z);
        sl.analysis_data(int).filename=data.(fn{:})(manual_final_insect.somewhat_likely(vli).y).filename;
        sl.analysis_data(int).dnum=exact_dnum;
        sl.analysis_data(int).datetime=exact_time;
        sl.analysis_data(int).frequency=fqdata;
        sl.analysis_data(int).freq_data=freq_data;
        sl.analysis_data(int).gauss_freq_data=g_freq_data;
        sl.analysis_data(int).max_peak=pk_freq;
        sl.analysis_data(int).positive_data=positive_data;
       
        
end

disp('done')
 save(fullfile(filename,'Training_Data','sl_analysis_data.mat'),'-struct','sl');
